...
role_for() {
    case "$1" in
        ...
        bin/start.sh)                                 echo "launcher|browser one-command entry point (cwd-robust; resolves its root from BASH_SOURCE); the attendee's ./bin/start.sh — same file the cloned repo ships, bundled so the extracted/installed layouts have it too" ;;
        bin/lock)                                      echo "launcher|cwd-robust CLI dispatcher over lock-tool.py / recover-baseline.py / build-injected.py; the attendee's ./bin/lock read|write|recover|build — dual-path tools probe (workshop/kit/tools in-repo, ./tools in kit/installed) so the one handout is correct everywhere" ;;
