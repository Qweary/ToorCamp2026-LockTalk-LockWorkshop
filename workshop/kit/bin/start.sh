...
# ---------------------------------------------------------------------------
print_fallback() {
    # The lock launcher is cwd-robust and sits beside this script in bin/, so the
    # one-liners below work from ANY directory (no need to be in the repo root).
    local lock_cmd="${REPO_ROOT}/bin/lock"
    echo
    echo "----------------------------------------------------------------------"
    echo "  CLI fallback — run these from anywhere (no browser needed):"
    echo "----------------------------------------------------------------------"
    echo "  READ  the codes:"
    echo "    ${lock_cmd} read --all"
    echo
    echo "  WRITE your own code:"
    echo "    ${lock_cmd} write --code 246810 --slot 25 --role supervisor"
    echo "----------------------------------------------------------------------"
    echo
}
