...
# not the whole byte. Across 14+ independent lock dumps (T+4e canonical baseline
# + the original T2/T3 research corpus) every valid user-code page has byte 0
# low-nibble == 0xD; factory/unmodified locks read 0xFD, but a keypad-
# reprogrammed lock (T+4g second board) reads 0x1D — same low nibble, different
# high nibble. The high nibble is a variable firmware status/sequence field
# (deep analysis DEFERRED to T+4h+); the low nibble 0xD is the stable page-type
# marker. See DUMP-VALIDATION.md §2 (2026-05-20, T+4g note). Blank (0x00) and
...
# Valid permission flag values per DUMP-VALIDATION.md §4:
...
# Slot N region map per DUMP-VALIDATION.md §4.
...
# Canonical workshop patch table — must match T+4e-INJECTION-PAYLOAD.md §1.
DEFAULT_PATCHES = [
...
def encode_code(code: str) -> tuple[int, int, int]:
    ...
    the AT45DB041E user-code-page encoding rule per DUMP-VALIDATION.md §3a; it
...
def main() -> None:
    ...
    print(f"Expected MD5 for the canonical workshop patch against the T+4e baseline:")
