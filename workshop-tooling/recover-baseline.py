...
Refuses to run unless the baseline file matches the T+4e-validated MD5 of the
intact-lock golden master, and unless the file is exactly 540,672 bytes (the
minipro main-array-only read scope per T+4e-DUMP-VALIDATION-REPORT.md §1 and
T48-KALI-PLAN.md §4.2). Defaults to verifying the write with a read-back-and-
compare; skip with --skip-verify if workshop time pressure demands.
...
# ---------------------------------------------------------------------------
# Constants — load-bearing, sourced from T+4e bench validation.
#
...
#   (T48-KALI-PLAN.md §4.2). Same constant used by build-injected.py.
#
# CANONICAL_BASELINE_MD5 — the T+4e operator-bench validated MD5 of the intact
#   in-service T2/T3 lock capture (T+4e-DUMP-VALIDATION-REPORT.md §header).
#   Two independent reads byte-identical to this MD5 on the bench; the same MD5
#   is the basis for build-injected.py's expected post-build MD5
...
#   that one baseline IS this canonical T+4e master.
#
...
#   (T+4e bench finding — T48-KALI-PLAN.md §1.4b).
#
# MINIPRO_FLAGS — '-i' is the standing skip-ID-check flag. The §3.4 dry probe
#   is the once-per-session ID-check confirmation; subsequent operations use
#   -i to avoid intermittent Chip-ID 0x0000 stalls on this AT45DB041E sample
#   against T48 firmware 00.1.34 (T+4f standing rule — T48-KALI-PLAN.md §1.5,
#   §3.4, §4.1, §5.4).
# ---------------------------------------------------------------------------
DUMP_SIZE = 540_672
...
def validate_baseline_size(data_len: int, path: str) -> None:
    ...
    T+4e-DUMP-VALIDATION-REPORT.md §4b and T48-KALI-PLAN.md §4.2; an attendee
    who staged the wrong baseline file from a non-minipro source will hit this
    and need the 'trim the trailing 128 bytes' guidance.
    """
...
def validate_baseline_md5(md5: str, path: str) -> None:
    """MD5 pre-flight — refuse to write anything other than the canonical T+4e
    ...
    sys.exit(
        ...
        f"(the T+4e canonical intact-lock baseline per "
        f"T+4e-DUMP-VALIDATION-REPORT.md). Refusing to write a wrong-baseline "
        f"file to the chip. Confirm you are pointing at the workshop-shipped "
        f"baseline at {CANONICAL_BASELINE_PATH}."
    )
...
def build_minipro_cmd(baseline_path: str, output_path: str | None = None,
    ...
    -i flag and the fully-qualified device name (T48-KALI-PLAN.md §3.4, §4.1).
    """
    if mode == "write":
...
def confirm_or_exit(baseline_path: str, baseline_md5: str, do_verify: bool) -> None:
    ...
    print(f"                       (matches canonical T+4e baseline — OK)")
...
def run_streaming(cmd: list[str], label: str) -> None:
    ...
    try:
        ...
        sys.exit(f"ERROR: {e}. Is minipro installed and on PATH? "
                 f"Per T48-KALI-PLAN.md Phase 1, the binary lives at "
                 f"/usr/local/bin/minipro.")
...
def self_test(baseline_path: str, auto_detected: bool) -> int:
    ...
    kit packaging (T+4g Task 4) exercises this as a stand-up check before the
    doors open. Runs the path-resolution contract gate first, then validates the
    resolved baseline file's size + MD5.
    """
    ...
    if md5 != CANONICAL_BASELINE_MD5:
        ...
        return 1
    print(f"  RESULT:              PASS — baseline is the canonical T+4e master.")
